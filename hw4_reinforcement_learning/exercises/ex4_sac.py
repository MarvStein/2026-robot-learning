...
class SACAgent:
    ...
    def sample_action(self, obs: torch.Tensor):
        ...
        """
        with torch.no_grad():
            # Sample an action from the actor for environment interaction.
            #
            # Hint:
    ...
    def compute_critic_loss(self, obs, act, rew, next_obs, done) -> torch.Tensor:
        ...
        """
        with torch.no_grad():
            # Compute the target Q value for SAC.
            #
            # Hint:
            ...
            # Flatten all to 1D for proper broadcasting
            target_q = (rew.flatten() + self.gamma * (1.0 - done.flatten()) * q_next.flatten())
    ...
    def soft_update_targets(self) -> None:
        ...
        with torch.no_grad():
            for target_param, param in zip(
                ...
                # foo.copy_(bar) copies from bar to foo
